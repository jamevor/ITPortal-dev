	...
	window.egu = new EntityGroupsUtility(
		{
			container: '#egu',
			entity: 'service',
			entityID: $('#service-id').length ? $('#service-id').val() : null,
			isNewEntity: !$('#service-id').length,
...
const saveHandler = async(stay = true) => {
	...
	const components = $('.component-card-item').map(function(index, value) {
		return $(value).data('componentid');
	}).toArray();
	const articles = $('.article-card-item').map(function(index, value) {
		return $(value).data('articleid');
	}).toArray();
	const portfolios = $('.portfolio-card-item').map(function(index, value) {
