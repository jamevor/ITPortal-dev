		...
		if (!$('#button-submit-feedback-form').hasClass('disabled')) {
			...
			grecaptcha.ready(function() {
				grecaptcha.execute('6LcW7ZYUAAAAAJqH2HgvLFroSHpBBv5xgNoP5MZD', {action: 'button_submit_feedback_form'}).then(function(token) {
					$.ajax(
						{
							url: '/api/v1/cherwell/ticket/create/one/changeOnboarding',
							method: 'POST',
							data: {
								token,
								formData: {
									source: window.location.href,
									type: 'Request Change of Onboarding Day',
							...
							error: function(resp) {
								...
								$('#button-submit-feedback-form').html('Submit Request for Change of Onboarding');
							}
						}
					);
				});
