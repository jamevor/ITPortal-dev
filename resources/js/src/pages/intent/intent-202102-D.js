...
$(document).ready(function() {
	...
	$('input[name=onlineOnly], input[name=comeToCampus]').change(function(){
		let online = $('input[name=onlineOnly]:checked').val();
		let campus = $('input[name=comeToCampus]:checked').val();
		let past = $('#pastDesignation').val();
		
		if($('#isResident').is(':checked')){
			if(past == "Remote - In Testing" || past == "Remote Learner" ){
				if(online =="No" || campus =="Yes"){
					$('#welcomeBack').slideDown()
	...
	$('input[name=moveInReturner]').change(function(){
		if($(this).val() == "Yes"){
			$('#arrivingFrom').slideDown()
			$('#housingDetails').hide()
			$('#housingDetails2').hide()
			$('#moveInReturnerYes').slideDown()
			$('#moveInReturnerNo').hide()
			
		}else{
			$('#arrivingFrom').hide()
			$('#housingDetails').slideDown()
			$('#housingDetails2').slideDown()
			$('#moveInReturnerNo').slideDown()
			$('#moveInReturnerYes').hide()
		}
	})
	...
	$('#button-submit-help-form').click(function(event) {
		...
		if($('#isResident').is(':checked') 
		&& $('input[name=moveInReturner]:checked').val() == "Yes"
				...
				grecaptcha.ready(function() {
					grecaptcha.execute('6LcW7ZYUAAAAAJqH2HgvLFroSHpBBv5xgNoP5MZD', {action: 'button_submit_help_form'}).then(function(token) {
						const formData = {
							...
							moveInReturner:$('input[name=moveInReturner]:checked').val(),
							previousDes:$('#pastDesignation').val(),
						};
