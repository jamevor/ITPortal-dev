		...
		if (!$('#button-submit-feedback-form').hasClass('disabled')) {
			...
			grecaptcha.ready(function() {
				grecaptcha.execute('6LcW7ZYUAAAAAJqH2HgvLFroSHpBBv5xgNoP5MZD', {action: 'button_submit_feedback_form'}).then(function(token) {
					$.ajax(
						{
							url: '/api/v1/cherwell/ticket/create/one/changeTest',
							method: 'POST',
							data: {
								token,
								formData: {
									source: window.location.href,
									type: 'Request Change of Test Day',
									fullName: $('#full-name').val() || $('#full-name').text(),
									wpiEmail: $('#wpi-email').val() || $('#wpi-email').text(),
									wpiID: $('#wpi-id').val() || $('#wpi-id').text(),
									frequency: $('#frequency').val(),
									currentDays: $('#chooseDay').val(),
									preferredDays: $('#switchDay').val(),
									reason: $('#reason').val()
								}
							},
							beforeSend: function() {
							...
							error: function(resp) {
								...
								$('#button-submit-feedback-form').html('Submit Request for Change');
							}
						}
					);
				});
